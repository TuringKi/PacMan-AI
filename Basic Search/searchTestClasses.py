...
class GraphSearchTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if solution in gold_solution and (not self.exactExpansionOrder or expanded_states in gold_expanded_states):
            ...
            grades.addSecretMessage('\tcorrect solution:\t\t%s' % gold_solution[0])
            grades.addSecretMessage('\tcorrect expanded_states:\t%s' % gold_expanded_states[0])
            grades.addSecretMessage('\tcorrect rev_solution:\t\t%s' % gold_solution[1])
            grades.addSecretMessage('\tcorrect rev_expanded_states:\t%s' % gold_expanded_states[1])
            return False
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if solution not in gold_solution:
            ...
            grades.addSecretMessage('\tcorrect solution:\n%s' % wrap_solution(gold_solution[0]))
            grades.addSecretMessage('\tcorrect (reversed) solution:\n%s' % wrap_solution(gold_solution[1]))
            return False
